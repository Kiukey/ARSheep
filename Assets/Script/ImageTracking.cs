...
/*            else
 ...
 /*   [SerializeField] ARTrackedImageManager imageManager = null;
    ...
    [SerializeField] GameObject sheepPrefab = null;
    GameObject sheepSpawned = null;
    void Start()
    {
    ...
    void UpdateImages(ARTrackedImagesChangedEventArgs obj)
    {
        foreach (ARTrackedImage image in obj.added)
        {
            if (image.referenceImage.guid == imageLibrary[0].guid)
            {
                sheepSpawned = Instantiate(sheepPrefab, image.transform.position, image.transform.rotation);
            }
        }
    }*/
